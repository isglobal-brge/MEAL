...
The function `correlationMethExprs` is useful when we have a list of differentially methylated probes and we want to check if they are correlated with expression probes. First, CpGs are paired to expression probes if the expression probe is in a range of $\pm$ 250kb from the CpG (this parameter is called flank). After that, for each pair, the following regression is performed: $$ e_i = \beta m_i + \epsilon_i $$ where $e_i$ are the expression values for individual i, $m_i$ the methylation values for individual i and $\beta$ the change of expression produced by the methylation.

Technical and biological covariates can be substracted from the methylation or the expression data. In the following lines, we will illustrate this feature using methylation data but the same procedure can be applied to expression data. To do the adjustment, a linear regression of the data versus the variables is performed: $$ x_{ij} = \sum_{k = 1}^p{\beta_{ik} z_{kj}} + r_{ij} $$ where $x_{ij}$ are the methylation values of CpG i for individual j, $\beta_{ik}$ is the effect of variable k at CpG i, $z_{kj}$ is the value of variable z for indivudal j and $r_{ij}$ are the residual values of CpG i and individual j. Then, the residuals are computed: $$ r_{ij} = x_{ij} - \sum_{k = 1}^p{\beta_{ik} z_{kj}} $$ These residuals will be used as the methylation values in the first equation. 
