...
Our function `runPipeline` includes the parameter `sva` that runs SVA and includes these variables as covariates in the linear model. Therefore, we will run again our analyses adjusting by SVs. We won't include cell counts as covariates as their effect will be included in the SVs :
...
QQplot still shows a S-shape. However, lambda is slightly closer to 1, so we will work with this model in the following steps. 
