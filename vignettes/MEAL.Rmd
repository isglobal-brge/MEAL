...
```{r getGeneVals}
getGeneVals(res, "ARMS2", genecol = "UCSC_RefGene_Name", fNames = c("chromosome", "start"))
```
...
```{r Manhattan 2}
plot(res, rid = "DiffMean", type = "manhattan", suggestiveline = 3, 
     genomewideline = 6)
```
...
```{r Regional plot 2}
plotRegion(res, targetRange, results = c("DiffMean", "bumphunter"), 
           tPV = 10)
```
