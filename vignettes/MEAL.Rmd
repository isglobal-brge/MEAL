...
`r Biocpkg("MEAL")` is meant to analyze methylation data already preprocessed. All our functions accept a `GenomicRatioSet` as input, which is a class from `r Biocpkg("minfi")` package designed to manage preprocessed methylation data. Users willing to preprocess their own data are encouraged to take a look to [`r Biocpkg("minfi")`'s vignette](http://bioconductor.org/packages/release/bioc/vignettes/minfi/inst/doc/minfi.pdf)

In this vignette, we will use methylation data from `r Githubpkg("isglobal/BRGEdata")` package. This package contains data from a spanish children cohort. We will load the required libraries and the `GenomicRatioSet` called `brge_methy` by typing: 
...
`brge_methy` contains measurements for 476946 CpGs and 115 samples, as well as some phenotypic variables such as age, sex or cell counts. To speed up the demonstration, we will work with a subset of 20 samples:

```{r subset Methy}
meth <- brge_methy[, 1:20]
```
...
The function `runPipeline` can run all methods included in `r Biocpkg("MEAL")` to the same dataset. We only need to pass to this function a `GenomicRatioSet` and the name of our variable of interest. In our case, we will analyze the effect of sex on methylation:

```{r Pipeline,  warning=FALSE}
res <- runPipeline(set = meth, variable_names = "sex")
res
```

`runPipeline` includes a lot parameters to customize the analyses. The most important parameters are `covariable_names`, `betas` and `sva`. `covariable_names` is used to include covariates in our models. `betas` allows the user choosing between running the analyis with beta (TRUE) or M-values (FALSE). If `sva` is TRUE, Surrogate Variable Analysis is run and surrogate variables are included in the models. Finally, some parameters modify the behaviour of the methods included in the wrapper and they will be covered in a posterior section. More information about the parameters can be found in the documentation (by typing ?runPipeline). 

We will run the analysis to exemplify how to add covariables in our models. To do so, we will pass the names of the cell counts measurements to `covariable_names`:

```{r Pipeline Adj,  warning=FALSE}
cellCounts <- colnames(colData(meth))[3:9]
cellCounts
resAdj <- runPipeline(set = meth, variable_names = "sex", 
                      covariable_names = cellCounts)
resAdj
```
...
We can use the function `getAssociation` to get a data.frame with the results, independent of the original method. This method has two main arguments: `object` and `rid`. `object` is the `ResultSet` with our data and `rid` is the name or the index of the analysis we want to extract. 
...
We will retrieve the difference in variance results for all CpGs mapped to BRCA1:

```{r getGeneVals}
getGeneVals(res, "BRCA1", genecol = "genes")
```
...
We will get obtain these three plots for the unadjusted analysis of differential means:

```{r 3 plots}
plot(res, rid = "DiffMean", type = "manhattan")
plot(res, rid = "DiffMean", type = "volcano")
plot(res, rid = "DiffMean", type = "qq")
```
...
plot(res, rid = "DiffMean", type = "manhattan", subset = targetRange)
```
...
```{r Manhattan 2}
plot(res, rid = "DiffMean", type = "manhattan", suggestiveline = 9, genomewideline = 15)
```
...
```{r Manhattan 3}
plot(res, rid = "DiffMean", type = "manhattan", main = "My custom Manhattan")
...
```{r Volcano 1}
plot(res, rid = "DiffMean", type = "volcano", tPV = 14, tFC = 0.4, show.labels = FALSE)
```

Volcano plot is based on `r CRANpkg("ggplot2")` so we can further customize the plot adding new layers:

```{r Volcano 2}
plot(res, rid = "DiffMean", type = "volcano", tPV = 30, tFC = 0.1, 
     show.labels = FALSE) + ggtitle("My custom Volcano")
```

### QQplot

Our QQplot include the computation of the lambda, a measure of the inflation of the p-values. We can remove this value with the parameter `

Our qqplot is also based on `r CRANpkg("ggplot2")` so we will add a title to customize it:

```{r QQ}
plot(res, rid = "DiffMean", type = "qq") + ggtitle("My custom QQplot")
```
...
In the next line, we will plot a CpG with high difference in means between male and female (cg17547524) and a CpG with high difference in variance (cg05111645) vs sex. As plotFeature is based on `r CRANpkg("ggplot2")`, we can customize it:

```{r Plot_Features, warning = FALSE}
plotFeature(set = meth, feat = "cg17547524", variables = "sex") + ggtitle("Diff Means")
plotFeature(set = meth, feat = "cg05111645", variables = "sex") + ggtitle("Diff Vars")
```

### Regional plotting

We can simultaneously plot of the different methods in a target region along with gene and CpG annotation with the function `plotRegion`. This function has two main arguments. `rset` is the `ResultSet` and `range` is a `GenomicRegion` with our target region. 
...
Our plot has three main parts. The top contains the annotation of the regional genes and the CpGs included in the analysis. The middle part contains the results of the DMR detection methods (Bumphunter, blockFinder and DMRcate). The bottom part contains the results of the single probe analyses (differential mean and differential variant). Each analysis has two parts: the coefficients and the p-values adjusted by FDR. A line with a significance 

By default,`plotRegion` includes all analyses run in the plot. However, we can plot only few analyses with the parameter `results`. We can also modify the height of the p-value line with the parameter `tPV`:

```{r Regional plot 2}
plotRegion(res, targetRange, results = c("DiffMean", "bumphunter"), tPV = 10)
```
...
```{r DiffMean}
resDM <- runDiffMeanAnalysis(set = meth, model = ~ sex)
```
...
```{r DiffVar}
resDV <- runDiffVarAnalysis(set = meth, model = ~ sex, coefficient = 2)
```
...
```{r Bumphunter}
resBH <- runBumphunter(set = meth, model = ~ sex, coefficient = 2)
```
...
```{r blockFinder}
resBF <- runBlockFinder(set = meth, model = ~ sex, coefficient = 2)
```
...
```{r DMRcate}
resDC <- runDMRcate(set = meth, model = ~ sex, coefficient = 2)
```
...
resRDA <- runRDA(set = meth, model = ~ sex, range = targetRange)
```
...
We can RDA along with our pipeline when we are previously interested in evaluating a target genomic range. In this case, we will pass our target region to the argument `range` of `runPipeline` and we can pass other parameters of `runRDA` using the argument `rda_params`.
...
We will plot RDA using sex variable of our sets colData:

```{r plotRDA}
plotRDA(object = resRDA, pheno = colData(meth)[, "sex", drop = FALSE])
```
